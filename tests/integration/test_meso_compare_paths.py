...
def test_compare_series_with_various_lengths():
    app = create_app('testing')
    _install_fake_provider()
    client = app.test_client()

    # refresh symbols
    client.post('/api/meso/refresh?period=1y', json={"symbols": ['^GSPC', '^NDX', '^STOXX50E']})

    # 1 symbol
    assert client.get('/api/meso/compare_series?symbols=%5EGSPC').status_code == 200
    # 5 symbols
    assert client.get('/api/meso/compare_series?symbols=%5EGSPC,%5ENDX,%5ESTOXX50E,%5EN225,%5EHSI').status_code == 200
